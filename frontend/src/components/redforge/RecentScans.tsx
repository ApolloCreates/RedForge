...
const STATUS_TONE: Record<string, { dot: string; text: string; label: string }> = {
  ...
  failed: { dot: "bg-primary", text: "text-primary", label: "Failed" },
    ...
    <section className="panel p-5">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="font-display text-sm font-bold tracking-widest uppercase">Recent Scans</h2>
        </div>
      ...
      {scans.length === 0 ? (
        ...
        <ul className="mt-3 divide-y divide-border">
          {scans.map((scan) => {
            const tone = STATUS_TONE[scan.status] ?? STATUS_TONE["queued"]!;
            const { date, time } = formatScanDate(scan.created_at);
              ...
              <li key={scan.id}>
                <button
                  ...
                  className={cn(
                    ...
                    selectable ? "hover:bg-accent/40" : "cursor-default",
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className={cn("mt-0.5 text-sm font-semibold", tone.text)}>{tone.label}</p>
